...
void logPower(int lightLevel) {
  ...
  String stateStr  = lightsAreOn ? "LIGHTS_ON" : "STANDBY";
...
void setup() {
  ...
  Serial.println("SmartLight Totem - Auto light sensor mode");
...
void loop() {
  ...
  // ── Light sensor control with hysteresis ──
  if (!lightsAreOn && lightLevel < TURN_ON_THRESHOLD) {
    lightsOn();
    setTopRing(255, 255, 255); // white = illuminating
